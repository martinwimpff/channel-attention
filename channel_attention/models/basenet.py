...
from .attention_blocks import get_attention_block
    ...
    def __init__(
        ...
        if attention_mode is not None:
            self.attention_block = get_attention_block(
                attention_mode, ch_dim=in_channels, reduction_rate=reduction_rate,
...
class BaseNetModule(nn.Module):
    def __init__(
        ...
        self.channel_attention_block = _ChannelAttentionBlock(
            ...
            pool_stride=pool_stride, dropout=dropout, reduction_rate=reduction_rate,
            use_mlp=use_mlp, seq_len=seq_lengths[0], freq_idx=freq_idx,
            n_codewords=n_codewords, kernel_size=kernel_size, extra_params=extra_params)
